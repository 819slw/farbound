...
//引入定义ajax请求方法
import {
  reqDeleteUser, reqAddUser, reqUserList, reqDepartList,reqRoleList
} from "../../../api"
...
export default {
  ...
  methods: {
    ...
    intoMuseum() {
      ...
      this.formFileds.depart_name=this.depart_name_map.get(this.formFileds.depart_id)
    },
    //搜索功能
    handleSearch(val) {
      let search = val;
      ...
      if (search != "") {
        ...
        this.infos = this.infos.filter(
          (data) =>
            !search || data.nick_name.toLowerCase().includes(search.toLowerCase())
        );
        this.infos_length = this.infos.length;
      }
    },
